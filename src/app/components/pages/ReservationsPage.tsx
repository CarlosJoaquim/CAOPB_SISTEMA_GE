      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {activeTab === 'reservations' ? (
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">
                    €{reservations.reduce((sum, r) => sum + parseFloat(r.preco_total), 0).toFixed(2)}
                  </p>
            ...
            <div className="bg-card rounded-xl p-6 border border-border">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">
                    {reservations.reduce((sum, r) => sum + r.convidados, 0)}
                  </p>
                  <p className="text-muted-foreground text-sm">Total Convidados</p>
                </div>
              <div className="bg-slate-900/40 backdrop-blur-md rounded-3xl p-6 border border-slate-800">
                <h3 className="text-lg font-black text-white mb-8 flex items-center gap-2">
                  <TrendingUp size={20} className="text-emerald-500" /> Faturamento por Local
            ...
            <div className="bg-card rounded-xl p-6 border border-border">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{restaurantStats.length}</p>
                  <p className="text-muted-foreground text-sm">Top Restaurantes</p>
                </div>
            {/* Tabela de Top Performance */}
            <div className="bg-slate-900/30 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl">
              <div className="px-6 py-5 border-b border-slate-800 bg-slate-900/50">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-card rounded-xl p-6 border border-border">
              <h3 className="text-lg font-semibold text-foreground mb-4">Top Restaurantes por Reservas</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={restaurantStats}>
                  ...
                  />
                  <Bar dataKey="totalReservas" fill="#06b6d4" />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-card rounded-xl p-6 border border-border">
              <h3 className="text-lg font-semibold text-foreground mb-4">Top Restaurantes por Receita</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={restaurantStats}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis
                  ...
                  />
                  <Bar dataKey="receitaTotal" fill="#10b981" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            <div className="px-6 py-4 border-b border-border">
              <h3 className="text-lg font-semibold text-foreground">Estatísticas Detalhadas dos Restaurantes</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-muted">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Restaurante</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Categoria</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Total Reservas</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Receita Total</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Ticket Médio</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {restaurantStats.map((restaurant, index) => (
                    <tr key={restaurant.nome} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-4">
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-800/30">
                    {restaurantStats.map((restaurant, index) => (
                      ...
                      <td className="px-6 py-4">
                        <p className="text-muted-foreground">{restaurant.categoria}</p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-foreground font-medium">{restaurant.totalReservas}</p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-foreground font-medium">
                          €{restaurant.receitaTotal.toFixed(2)}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-foreground font-medium">
                          €{(restaurant.receitaTotal / restaurant.totalReservas).toFixed(2)}
                        </p>
                      </td>
                    </tr>
                  ))}
